...
const PATIENT_COUNT_API = `${API_BASE}/api/appointments/patients/count`;  //fetching from the backend route

//Helper functions

//ot will return a finite number
  ...
  //to load doctors from the server
   useEffect(() => {
     ...
     async function loadDoctors() {
       ...
       setError(null);
       try {
         const url = `${API_BASE}/api/doctors?limit=200`;  // to get doctors from backend route
  ...
  return (
    <div className={s.mobileDoctorCard}>
      <div className={s.mobileDoctorHeader}>
        ...
        <div className={s.mobileDoctorFee}> Ruppe {d.fee}</div>
      </div>
      ...
      <div className={s.mobileEarningsContainer}>
        <div>Earned</div>
        <div className="font-semibold"> Ruppe {d.earnings.toLocaleString()}</div>
      </div>
    </div>
