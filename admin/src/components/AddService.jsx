...
const AddService = ({serviceId}) => {
  ...
  const [hasExistingImage, setHasExistingImage] = useState(false); //to update the image if already exists
  const [removeImage, setRemoveImage] = useState(false); //to remove existing image
  ...
  //Date/time control for adding slots
  const now = new Date();
  ...
  //compute thge days for selected month/year (respecting the month length)
  ...
  //prevent user from selecting past time for that particular date
  function isSelectedDateTimeInPast() {
    const sel = selectedDateTime();
    return sel.getTime() <= Date.now();
  }

  //to add or update the slots
  function addSlot() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const url = serviceId
        ? `${API_BASE}/api/services/${serviceId}`//to update service
        : `${API_BASE}/api/services`;// to create a service
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
          <div>
            ...
            <p className={addServiceStyles.header.subtitle}>
              Create a beautiful service card with unique times slots
            </p>
          </div>
