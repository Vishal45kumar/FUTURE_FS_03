...
function RescheduleButton({ appointment, onReschedule, disabled }) {
  ...
  //to save the appointments after editing
  function save() {
    if (!date || !time) return;
...
const ServiceAppointmentPage = () => {
    ...
    //to reschedule a particular service appointment for later but not on previous days
    ...
    async function cancelRemote(id) {
      ...
      try {
        ...
        pushToast(
          "Cancel failed",
          err.message || "Failed to cancel — reloading",
        );
        await fetchAppointments();
      }
    }
    ...
    //sort the upcoming date comes first
      ...
      <header className={serviceAppointmentsStyles.headerContainer}>
        <div className={serviceAppointmentsStyles.headerTitleContainer}>
          ...
          <p className={serviceAppointmentsStyles.headerSubtitle}>
            Manage patient bookings - quick serach & status controls
          </p>
        </div>
