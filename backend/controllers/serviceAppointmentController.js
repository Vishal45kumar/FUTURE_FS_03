...
//this function will pass time and return AM/Pm
...
export const confirmServicePayment = async(req,res)=>{
    try {
      ...
      console.error("confirmService payment error", err);
      return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const updateServiceAppointment = async(req,res)=>{
    try {
      ...
      //first check whether it is filled if yes update all the fields
...
export const getServiceAppointmentsBytPatient = async (req,res)=>{
  try {
    const clerkUserId = resolveClerkUserId(req);
    ...
    console.error("getServiceAppointmentsBytPatient error", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export default {
  ...
  getServiceAppointmentsBytPatient
}