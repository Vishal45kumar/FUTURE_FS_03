...
function resolveClerkUserId(req) {
  return (
    req.userId || // ✅ ADD THIS (your middleware)
    req.auth?.userId || // Clerk middleware
    req.auth?.user_id ||
    req.auth?.user?.id ||
    req.user?.id ||
    getAuth(req)?.userId ||
...
export const getAppointments = async(req,res)=>{
    try{
        ...
        console.error("GetAppintmentError:",err)
...
try {
  ...
  console.error("GetAppintmentByPatient:", err);
  return res.status(404).json({
    success: false,
    message: "Server Error",
  });
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const numericFee = safeNumber(fees ?? fees ?? 0);
    ...
    try {
      ...
      console.warn("Doctor lookup field: ", err?.message || err);
    }
    if (!doctor) {
      return res.status(404).json({
        success: false,
        message: "Doctor not found error",
      });
    }
  ...
  try {
    ...
    if (session.payment_status !== "paid")
      return res.satus(404).json({
        success: false,
        message: "User has not not done the payment",
      });
    // confirmPayment
...
//to update a particular appointment
...
export const getStats = async (req,res) =>{
  try {
    ...
    console.error("gaetStats error error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export async function getRegisteredUserCount(req,res) {
  try {
    ...
    console.error(" get registered user count error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
