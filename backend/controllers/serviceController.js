...
//function takes date-time slot and group them into a YYYY-MM-DD with the time
...
export async function createService(req,res){
    try
    {
         ...
         } //if the file is present it wil uploaded to services folder in the Cloudinary
...
export async function updateService(req,res){
    try {
        ...
        try {
          ...
          if (up?.secure_url) {
            ...
            if (existing.imagePublicId) {
              //it will the old image and replace with new image
              try {
      ...
      console.error("UpdateServiceError error:", error);
      return res.status(500).json({
        success: false,
...
export async function deleteService(req,res){
    try {
      ...
      console.error("DeleteServiceError error:", error);
      return res.status(500).json({
        success: false,
        message: "Server error",
      });
