...
//convert time 12 hrs to number of minutes since midnight and return am : pm
...
//remove the duplicate slots and return the slot filter by time
...
//convert doctor data into plain text
...
export const getDoctors = async (req, res) => {
  try {
    const { q = "", limit: limitRaw = 200, page: pageRaw = 1 } = req.query; // 200 doctors per page
    ...
    const total = await Doctor.countDocuments(match); //count total number of doctors
    return res.json({
      success: true,
      data: normalized,
      doctors: normalized,
...
export async function getDoctorById(req, res) {
  try {
    ...
    return res.status(505).json({ success: false, message: "Server error" });
  }
}

//to update a particular doctor
  ...
  try {
    ...
    if (!existing)
      return res
        ...
        //if image exists and we want to update the image
    if (req.file?.path) {
      const uploaded = await uploadToCloudinary(req.file.path, "doctors");
      if (uploaded) {
...
export async function toggleAvailability(req,res){
    try {
      ...
      console.error("toggleavailablity error:", err);
      return res.status(500).json({ success: false, message: "Server error" });
    }
}
